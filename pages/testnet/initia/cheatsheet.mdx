...
initiad tx staking delegate $(initiad keys show wallet --bech val -a) 1000000000000000000mpx --from wallet --chain-id initiation-1 --gas-adjustment 1.5 --gas auto --fees 80000uinit -y
```
```js filename="Delegate token to other validator, change <to-valoper-address> as you like"
initiad tx staking delegate <to-valoper-address> 1000000000000000000mpx --from wallet --chain-id initiation-1 --gas-adjustment 1.5 --gas auto --fees 80000uinit -y
```
```js filename="Redelegate to another validator"
initiad tx staking redelegate $(initiad keys show wallet --bech val -a) <to-valoper-address> 1000000000000000000mpx --from wallet --chain-id initiation-1 --gas-adjustment 1.5 --gas auto --fees 80000uinit -y
```
```js filename="Unbond token from your own validator"
initiad tx staking unbond $(initiad keys show wallet --bech val -a) 1000000000000000000mpx --from wallet --chain-id initiation-1 --gas-adjustment 1.5 --gas auto --fees 80000uinit -y
```
```js filename="Send token to the wallet"
initiad tx bank send wallet <to-wallet-address> 1000000000000000000mpx --from wallet --chain-id initiation-1 --gas-adjustment 1.5 --gas auto --fees 80000uinit -y
```
## Governance
...
echo $(initiad tendermint show-node-id)'@'$(curl -s ifconfig.me)':'$(cat $HOME/.mineplex-chain/config/config.toml | sed -n '/Address to listen for incoming connection/{n;p;}' | sed 's/.*://; s/".*//')
```
...
initiad tendermint unsafe-reset-all --keep-addr-book --home $HOME/.mineplex-chain --keep-addr-book
```
